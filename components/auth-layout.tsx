...
export function AuthLayout({ children, title, description, view }: AuthLayoutProps) {
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center p-4 sm:p-8 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl bg-zinc-950/40 backdrop-blur-2xl rounded-[2rem] shadow-2xl overflow-hidden border border-white/5 lg:grid lg:grid-cols-2">
        
        {/* LADO IZQUIERDO: FORMULARIO */}
        <div className="flex flex-col justify-center px-6 py-10 sm:px-12 lg:px-14 relative z-10">
          <div className="mx-auto w-full max-w-md">
            
            <Link href="/" className="group flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-primary mb-8 transition-colors w-fit">
            ...
            {view && (
              <div className="flex p-1 mb-8 rounded-full bg-secondary/30 border border-white/5 backdrop-blur-md">
                {view === "login" ? (
                  <>
                    <div className="flex-1 text-center py-2 text-sm font-medium rounded-full bg-background/50 shadow-sm text-foreground">
                      Iniciar sesión
                    </div>
                    <Link href="/auth/register" className="flex-1 text-center py-2 text-sm font-medium rounded-full text-muted-foreground hover:text-foreground transition-colors">
                      Crear cuenta
                    </Link>
                  </>
                ) : (
                  <>
                    <Link href="/auth/login" className="flex-1 text-center py-2 text-sm font-medium rounded-full text-muted-foreground hover:text-foreground transition-colors">
                      Iniciar sesión
                    </Link>
                    <div className="flex-1 text-center py-2 text-sm font-medium rounded-full bg-background/50 shadow-sm text-foreground">
                      Crear cuenta
        ...
        <div className="hidden lg:flex relative items-center justify-center p-12 overflow-hidden bg-primary/5 border-l border-white/5">
          <div className="relative z-20 flex flex-col items-center text-center max-w-sm">
            ...
            <p className="text-zinc-300 text-sm leading-relaxed bg-black/20 backdrop-blur-md p-4 rounded-2xl border border-white/5">
              Un entorno creado para estudiantes de TI enfocado en llevar tus proyectos al siguiente nivel.
            </p>
          </div>
        </div>
